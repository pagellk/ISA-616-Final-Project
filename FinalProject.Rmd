...
# Filter the result to only include academic years from 2020-2021 onward
result <- result |>
...
graduated_students <- BA |>
  dplyr::filter(graduated == TRUE, Term.Code < 202510) |>
...
ggplot2::ggplot(graduated_students, ggplot2::aes(x = Cum.Credit.Hrs)) +
  ggplot2::geom_histogram(binwidth = 5, color = "black", fill = "#ADD8E6", alpha = 0.7) +
  ggplot2::labs(
...
graduated_students <- BA |>
  dplyr::filter(graduated == TRUE, !is.na(Cum.Credit.Hrs), Term.Code.Graduated < 202510)
...
## Graduation Term Mapping for IS Majors
```{r, echo=FALSE}
result <- graduated_students |>
  dplyr::group_by(`Term.Code`) |>
  dplyr::summarize(Num_Graduated = dplyr::n_distinct(`Student.ID`), .groups = 'drop') |>
...
IS <- IS %>%
  ...
  mutate(
    ...
    grade_classification = case_when(
      ...
      num_semesters_up_to_now >= 5 & num_semesters_up_to_now <= 6 ~ "Junior",
      num_semesters_up_to_now >= 7 & num_semesters_up_to_now <= 8 ~ "Senior",
      num_semesters_up_to_now > 8 ~ "Super-Senior",
      TRUE ~ "Unknown"
    )
  ) %>%
  ungroup()
...
# Step 3: Filter for students who have graduated and capture their cumulative credit hours at graduation
graduated_students <- IS |>
  dplyr::filter(graduated == TRUE, Term.Code.Graduated < 202510) |>
...
knitr::kable(result, col.names = c("Term Code", "Classification", "Number of Students"), caption = "Student Classification for Spring 2024") |>
  ...
  kableExtra::row_spec(0, background = "#ADD8E6")  # Light gray header
```
Add a column to IS data set that keeps track of a student's grade classification.
```{r, echo=FALSE}
# Step 1: Ensure unique term codes for each student
IS_unique_terms <- IS |>
  dplyr::distinct(Student.ID, Term.Code, .keep_all = TRUE)
