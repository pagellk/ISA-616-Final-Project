...
This preliminary analysis shows the number of students from each cohort term who graduated each semester. From first looks, I notice that most students graduate during the spring of their senior year. 
```{r, echo =FALSE}
...
# Create a mapping of Term.Code to academic year
result <- result |> 
  dplyr::mutate(Academic_Year = dplyr::case_when(
...
# Create a bar plot
ggplot2::ggplot(result, ggplot2::aes(x = Academic_Year, y = Num_Graduated)) +
  ggplot2::geom_bar(stat = "identity", fill = "skyblue") +
  ggplot2::geom_text(ggplot2::aes(label = Num_Graduated), vjust = -0.3, size = 3.5) +
  ggplot2::labs(
