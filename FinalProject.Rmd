...
Let's more closely take a look at those who graduated last spring:
...
# Step 1: Filter for graduated students and ensure non-missing credit hours
graduated_students <- IS %>% 
  filter(graduated == TRUE & !is.na(Cum.Credit.Hours))  # Only include graduated students with valid credit hours
...
Let's examine term code 202420, referencing Spring Semester 2024: 
...
result <- IS |>
  ...
  dplyr::mutate(Classification = dplyr::case_when(
    Cum.Credit.Hours >= 0 & Cum.Credit.Hours < 30 ~ "Freshmen",
    Cum.Credit.Hours >= 30 & Cum.Credit.Hours < 60 ~ "Sophomore",
    Cum.Credit.Hours >= 60 & Cum.Credit.Hours < 90 ~ "Junior",
    Cum.Credit.Hours >= 90 ~ "Senior"
  )) |>
...
We will proceed in furthering our analyses by classifying student grade levels by the number of semesters they have completed at Miami.

```{r, echo=FALSE}
## create a new column in IS to classify students based upon # of semesters they have completed.
